...
export const bhpIndex = async (req, res) => {
  try {
    ...
    res.status(500).render('error', {
      ...
      message: 'Gagal memuat daftar BHP.',
      statusCode: 500,
    })
  }
}
...
export const updateBhpStock = async (req, res) => {
  ...
  try {
    ...
    req.session.flash = { success: `Stok "${consumable.name}" berhasil diperbarui menjadi ${newStock}.` }
    res.redirect('/staff/bhp')
  } catch (err) {
    console.error('Staff update BHP stock error:', err)
    req.session.flash = { error: 'Gagal memperbarui stok BHP.' }
...
export const maintenanceIndex = async (req, res) => {
  try {
    ...
    res.status(500).render('error', {
      title: 'Kesalahan Server',
      message: 'Gagal memuat log maintenance.',
      statusCode: 500,
    })
  }
}
...
/**
 * POST /staff/maintenance/log
 * Create new maintenance log, update asset condition, and decrease BHP stock if used.
 */
export const storeMaintenanceLog = async (req, res) => {
  const { inventoryId, description, condition, consumableId, consumableQty } = req.body
  const invId = parseInt(inventoryId, 10)
  ...
  try {
    ...
    // Run transaction
    await prisma.$transaction(async (tx) => {
      // 1. If BHP is used, validate and deduct stock
      if (consId && consQty) {
        const consumable = await tx.consumable.findUnique({ where: { id: consId } })
        if (!consumable) {
        ...
        if (consumable.stock < consQty) {
          throw new Error(`Stok BHP "${consumable.name}" tidak mencukupi (Tersedia: ${consumable.stock}).`)
        }

        // Deduct stock
        await tx.consumable.update({
          where: { id: consId },
          data: { stock: consumable.stock - consQty },
        })
      }
